...
class PosCmd(metaclass=Metaclass_PosCmd):
    ...
    def __init__(self, **kwargs):
        ...
        if 'position' not in kwargs:
            self.position = numpy.zeros(32, dtype=numpy.float64)
        else:
            self.position = kwargs.get('position')
        self.traj_time = kwargs.get('traj_time', float())
        self.gravity = kwargs.get('gravity', bool())
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 32, \
                    "The 'position' numpy.ndarray() must have a size of 32"
    ...
    def gravity(self, value):
        if self._check_fields:
            assert \
                ...
                "The 'gravity' field must be of type 'bool'"
        self._gravity = value
