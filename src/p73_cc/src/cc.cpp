...
{
    cout << "[p73_cc] Initializing variables (VISION student mode, 70D obs / 25D sparse cmd)" << endl;
    ...
    // Initialize motion_cmd_ to an in-distribution standing reference (not zeros).
    // All-zeros is out-of-distribution for the sparse student command.
    // This is only a brief fallback: motion_cmd_publisher.py continuously publishes
    // the standing command on /p73/motion_cmd, overriding this within one cycle.
    // 25D sparse layout: root_vel_xy(2) + root_z(1) + roll(1) + pitch(1) + yaw_rate(1)
    //   + L/R foot_pos(6) + L/R foot_ori_Rot6D(12) + waist_yaw(1).
    // Values = frame-0 standing pose of p73_walk1_subject5 (matches publisher standing_cmd).
    motion_cmd_ = {
        0.00000, 0.00000, 0.89082, 0.01291, 0.03823, 0.00000,        // root6
        -0.04539, 0.11889, -0.81843, -0.04615, -0.13214, -0.81265,   // feet_pos (L xyz, R xyz)
        0.99852, -0.05100, 0.01899, 0.05102, 0.99870, -0.00014,      // L foot Rot6D
        0.99982, 0.00521, 0.01844, -0.00532, 0.99997, 0.00607,       // R foot Rot6D
        -0.03303,                                                    // waist_yaw
    };
}
...
// =====================================================================
// processObservation — 70D per frame (vision student policy)
//
// Layout:
//   [0:3]   base_ang_vel           3D
//   [3:6]   projected_gravity      3D
//   [6:31]  sparse_command         25D  (from /p73/motion_cmd topic)
//            root6 + feet_pos6 + feet_ori_Rot6D12 + waist1
//   [31:44] joint_pos_rel          13D  (ALL joints incl. WaistYaw)
//   [44:57] joint_vel              13D  (ALL joints, clip ±30, /30)
//   [57:70] last_action            13D  (raw network output, legs + WaistYaw)
// =====================================================================
void CustomController::processObservation()
{
    ...
    // sparse_command (25D) from /p73/motion_cmd topic
    {
    ...
    // Frame-major history: [frame0(70D), frame1(70D), ..., frame(H-1)(70D)]
...
void CustomController::computeFast()
{
    ...
    if ((control_time_us - time_inference_pre_) / 1.0e6 >= policy_dt_) {
        ...
        if (policy_step_count <= dump_max_steps) {
            // 70D frame: 6 terms
            constexpr int dims[] = {3, 3, 25, 13, 13, 13};
            const char* term_names[] = {"ang_vel", "gravity", "sparse_cmd",
                                        "joint_pos", "joint_vel", "last_action"};
            constexpr int num_terms = 6;
            int H = history_length_;

            // Extract newest frame (70D) from frame-major buffer
...
void CustomController::startSubscribers()
{
    ...
    // Sparse student command subscriber (25D Float64MultiArray)
    ...
    cout << "[p73_cc] Motion command subscriber started on topic: /p73/motion_cmd (25D Float64MultiArray)" << endl;
