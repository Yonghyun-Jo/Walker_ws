...
public:
    ...
    //////////////////////// Network Dimensions ////////////////////////
    // Vision student policy (student_mimic_env_cfg.py StudentPolicyCfg):
    //   base_ang_vel(3) + projected_gravity(3) + sparse_command(25)
    //   + joint_pos(13) + joint_vel(13) + last_action(13) = 70
    static const int num_action = 13;         // legs(12) + WaistYaw(1)
    static const int num_single_obs = 70;
    static const int num_motion_cmd = 25;     // sparse student command dim
    ...
    // Sparse student command (updated by ROS2 subscriber on /p73/motion_cmd)
    // 25D: root_vel_xy(2) + root_z(1) + roll(1) + pitch(1) + yaw_vel(1)
    //      + feet_pos(6, L/R xyz) + feet_ori_Rot6D(12, L/R) + waist_yaw(1)
    std::mutex motion_cmd_mutex_;
    std::array<double, num_motion_cmd> motion_cmd_{};
