...
Vision student: the exported ONNX takes term-major history [1, 700] (70D × 10 frames)
and outputs [1, 13] actions. Architecture is identical to the motion student
(normalizer → encoder → cat(current_obs, latent) → actor); only the dims differ.
NOTE: the canonical exporter is the workspace onnx_exporter.py (used by play); this
standalone script mirrors it for direct checkpoint→ONNX export.
"""
...
class FullPolicyWrapper(nn.Module):
    ...
    Input:  obs_history [batch, 700]  (term-major, 70D × 10 frames)
    Output: actions     [batch, 13]
    """
    ...
    def forward(self, obs_history: torch.Tensor) -> torch.Tensor:
        ...
        # 4. Concatenate [current_obs(70), latent(128)] = 198
...
def main():
    ...
    parser.add_argument("--num-single-obs", type=int, default=70, help="Single frame obs dimension")
    parser.add_argument("--history-length", type=int, default=10, help="History length")
    ...
    # Vision/motion student uses swish (SiLU) activation (rsl_rl_mimic_cfg activation="swish").
    # NOTE: this state_dict-rebuild path must hardcode the activation; the canonical
    # workspace onnx_exporter.py deepcopies the real modules and avoids this fragility.
    def build_mlp(prefix: str, activation=nn.SiLU):
