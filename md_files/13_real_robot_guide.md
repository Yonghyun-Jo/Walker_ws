...
## STEP 2. 기존 p73_walker_cc 백업 + 교체

```bash
# 백업
cp -r ~/Walker_ws/src/p73_walker_cc ~/Walker_ws/src/p73_walker_cc.bak

# 내용만 덮어쓰기 (p73_walker_cc 이외 패키지는 절대 건드리지 않음)
...
s = ort.InferenceSession('$HOME/Walker_ws/src/p73_walker_cc/policy/policy.onnx')
...
```bash
cp /media/<USB>/policy.onnx ~/Walker_ws/src/p73_walker_cc/policy/policy.onnx
```
...
# 2) p73_controller가 0.x초로 끝나면 캐시된 것 → 강제 재빌드
...
# → ~1분 걸려야 정상. 0.x초면 아직 캐시된 것.

# 3) 빌드 후 반드시 source
source install/setup.bash
```

> **왜 p73_controller도 빌드?**
> `p73_walker_cc`는 정적 라이브러리(.a)이고 `p73_controller`가 이걸 링크해서 실행 파일을 만듦.
> cc 코드를 바꿔도 controller를 재빌드 안 하면 **이전 코드가 계속 실행됨.**
...
콘솔에서 확인할 것:
```
[p73_walker_cc] Mode started (is_on_robot=1)   ← 1이어야 함
[p73_walker_cc] Inferred policy_obs_dim=235     ← ONNX 정상 로드
```
...
```bash
# 백업에서 복구
rm -rf ~/Walker_ws/src/p73_walker_cc
mv ~/Walker_ws/src/p73_walker_cc.bak ~/Walker_ws/src/p73_walker_cc
